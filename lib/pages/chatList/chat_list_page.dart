...
class ChatListPage extends StatefulWidget {
 // final String userID;
  ...
  const ChatListPage({super.key, required this.address});
...
class _ChatListPageState extends State<ChatListPage> {
  ...
  final snapshot = await FirebaseFirestore.instance.collection('ChatRoomInfo').get();
        ...
        children: filters.entries.map((entry) {
          ...
          return ElevatedButton(
            style: ElevatedButton.styleFrom(
              backgroundColor: isSelected ? AppColor.myProfilColor: Colors.grey[300],
              foregroundColor: isSelected ? AppColor.blackTextColor : Colors.black,
            ),
            onPressed: () => setState(() {
              filter = entry.value;
              selectedCategory = '';
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          SizedBox(height: 4),
                          Text(
                            room.descript.length > 20  //상세내용이 길면 20자 까지만 출력하고 '...'으로 줄임
